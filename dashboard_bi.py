...
def aplicar_rotulos_percentuais_barras(fig, limiar_outside=15.0, tamanho_padrao=12):
    """Padroniza rótulos (%) em gráficos de colunas (rankings) conforme regra final.
    ...
    2) Fora do padrão — barras < limiar_outside (ex.: Telmex):
       - NÃO mostrar % dentro
    ...
    Observação:
      - Usamos o valor do % como proxy robusto para quando o Plotly "aperte" o rótulo.
      - Ajustamos folga no eixo Y para não cortar texto 'outside'.
    """
    try:
        # Evita corte e cria folga no topo
        ...
        for tr in fig.data:
            ...
            # Formato sempre: 88% (número antes do %)
            ...
            # Vertical (cima → baixo): usar -90 (mantém leitura de cima para baixo)
