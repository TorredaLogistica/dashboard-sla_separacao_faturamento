...
METAS_NET = {"01/2026": 90.00, "02/2026": 90.00, "03/2026": 90.00, "04/2026": 90.00, 
             "05/2026": 90.00, "06/2026": 90.00, "07/2026": 90.00, "08/2026": 92.00,
             "09/2026": 92.00, "10/2026": 92.00, "11/2026": 92.00, "12/2026": 92.00}
METAS_CLARO_TV = {"01/2026": 85.02, "02/2026": 85.11, "03/2026": 85.19, "04/2026": 85.04,
                  "05/2026": 84.80, "06/2026": 84.90, "07/2026": 85.19, "08/2026": 84.77,
                  "09/2026": 84.97, "10/2026": 84.97, "11/2026": 85.05, "12/2026": 84.97}
METAS_EMBRATEL = {"01/2026": 80.00, "02/2026": 80.00, "03/2026": 80.01, "04/2026": 80.02,
                  "05/2026": 80.01, "06/2026": 79.99, "07/2026": 79.99, "08/2026": 82.00,
                  "09/2026": 81.98, "10/2026": 82.01, "11/2026": 82.00, "12/2026": 82.00}
METAS_CLARO_MOVEL = {"01/2026": 99.50, "02/2026": 99.50, "03/2026": 99.50, "04/2026": 99.50,
                     "05/2026": 99.50, "06/2026": 99.50, "07/2026": 99.50, "08/2026": 99.50,
                     "09/2026": 99.50, "10/2026": 99.50, "11/2026": 99.50, "12/2026": 99.50}
...
def _parse_data_log_sf(serie: pd.Series) -> pd.Series:
    """Converte datas do log sem inverter mês/dia.

    - ISO: YYYY-MM-DD HH:MM:SS -> yearfirst=True.
    - BR: DD/MM/YYYY HH:MM:SS -> dayfirst=True.
    - Segurança: corrige datas ambíguas antigas quando a alternativa está próxima da data atual.
    """
    ...
    if mask_iso.any():
        resultado.loc[mask_iso] = pd.to_datetime(s.loc[mask_iso], errors="coerce", yearfirst=True)

    mask_br = s.str.match(r"^\d{2}/\d{2}/\d{4}", na=False)
    if mask_br.any():
        data_br = pd.to_datetime(s.loc[mask_br], errors="coerce", dayfirst=True)
        data_alt = pd.to_datetime(s.loc[mask_br], errors="coerce", dayfirst=False)
        try:
            agora_sp = pd.Timestamp(datetime.now(ZoneInfo(FUSO_HORARIO_LOG)).replace(tzinfo=None))
            diff_br = (data_br - agora_sp).abs()
            diff_alt = (data_alt - agora_sp).abs()
            usar_alt = data_br.notna() & data_alt.notna() & (diff_br > pd.Timedelta(days=30)) & (diff_alt <= pd.Timedelta(days=10))
            data_br.loc[usar_alt] = data_alt.loc[usar_alt]
        except Exception:
            pass
        resultado.loc[mask_br] = data_br

    mask_restante = resultado.isna() & s.ne("")
    if mask_restante.any():
        resultado.loc[mask_restante] = pd.to_datetime(s.loc[mask_restante], errors="coerce", dayfirst=True)
...
def _normalizar_log_sf(df_log: pd.DataFrame) -> pd.DataFrame:
    ...
    df_log["Data"] = _parse_data_log_sf(df_log["Data"])
...
def render_identificacao_usuario_sf():
    ...
    if st.button("Acessar indicador", width="stretch"):
...
def render_dashboard_acessos_sf():
    ...
    if github_sf_configurado():
        sincronizar_agora = st.button("🔄 Sincronizar log com GitHub agora", width="stretch")
    ...
    if df_filtrado.empty:
        ...
        st.dataframe(ranking_usuario, width="stretch", hide_index=True)
        ...
        st.dataframe(ranking_visualizacao, width="stretch", hide_index=True)
        ...
        st.download_button("⬇️ Baixar histórico filtrado em CSV", data=csv_download, file_name=LOG_ACESSOS_SF_CSV, mime="text/csv", width="stretch")

    if os.path.exists(LOG_ACESSOS_SF_XLSX):
        with open(LOG_ACESSOS_SF_XLSX, "rb") as f:
            st.download_button("⬇️ Baixar histórico completo em Excel", data=f, file_name=LOG_ACESSOS_SF_XLSX, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", width="stretch")
    ...
    with st.expander("🛡️ Administração do log - apagar registros"):
        ...
        if st.button("🗑️ Apagar todos os registros", width="stretch"):
...
with st.sidebar:
    if os.path.exists("logo_claro.png"):
        st.image("logo_claro.png", width="stretch")
...
if total > 0:
    ...
    st.dataframe(view_display.style.apply(estilo_tabela, axis=1), width="stretch", hide_index=True)
    ...
    try:
        ...
        pass
    st.plotly_chart(fig_bar_cd, width="stretch", theme=None)
    # 2. RANKING EMPRESAS
    ...
    try:
        ...
        pass
    st.plotly_chart(fig_bar_emp, width="stretch", theme=None)
    ...
    try:
        ...
        pass
    st.plotly_chart(fig_bar_canal, width="stretch", theme=None)
    # =============================
